...
Tapiserie*** copiereinMatrice(Tapiserie* tapiserii, int n, int* nrLinii, int** nrElemLinie) {
    //Numaram cate dimensiuni sunt in tapiseriile din vector
    //Aflam care este dimensiunea maxima dintre tapiserii
    int nrDimensiuni = 0;
    for(int i=0; i<n; i++) {
        nrDimensiuni = tapiserii[0].nrbucati;
        if(nrDimensiuni < tapiserii[i].nrbucati) {
           nrDimensiuni= tapiserii[i].nrbucati;
        }
    }

    
    Tapiserie*** matrice= (Tapiserie***)malloc(sizeof(Tapiserie**) * nrDimensiuni);
    for(int i = 0; i < nrDimensiuni; i++) {
        matrice[i] = NULL;
    }
    
    *nrElemLinie= (int*)calloc(nrDimensiuni, sizeof(int));
    *nrLinii= nrDimensiuni;
    if(matrice == NULL || nrElemLinie == NULL) {
        printf("Eroare la alocarea memoriei\n");
        exit(1);
    }
    //Parcurgem vectorul de tapiserii
    for(int i= 0; i< n; i++) {
        int linie = tapiserii[i].nrbucati - 1;
        int poz = (*nrElemLinie)[linie];
        (*nrElemLinie)[linie]++;
        matrice[linie] = (Tapiserie**)realloc(matrice[linie], sizeof(Tapiserie*) * (poz + 1));
        matrice[linie][poz] = (Tapiserie*)malloc(sizeof(Tapiserie));
        if(matrice[linie] == NULL || matrice[linie][poz] == NULL) {
            printf("Eroare la alocarea memoriei\n");
            exit(1);
        }
        //Deep copy
        //Alocam memorie pentru denumire si culoare
        matrice[linie][poz]->id = tapiserii[i].id;
        matrice[linie][poz]->denumire = (char*)malloc(strlen(tapiserii[i].denumire) + 1);    
        matrice[linie][poz]->culoare = (char*)malloc(strlen(tapiserii[i].culoare) + 1);
        if(matrice[linie][poz]->denumire == NULL || matrice[linie][poz]->culoare == NULL) {
            printf("Eroare la alocarea memoriei\n");
            exit(1);
        }
        strcpy(matrice[linie][poz]->denumire, tapiserii[i].denumire);
        strcpy(matrice[linie][poz]->culoare, tapiserii[i].culoare);
        matrice[linie][poz]->pret = tapiserii[i].pret;
        matrice[linie][poz]->nrbucati = tapiserii[i].nrbucati;
        //Alocam memorie pentru dimensiuni
        matrice[linie][poz]->dimensiuni = (float*)malloc(tapiserii[i].nrbucati * sizeof(float));
        if(matrice[linie][poz]->dimensiuni == NULL) {
            printf("Eroare la alocarea memoriei\n");
            exit(1);
        }
        for(int j=0; j<tapiserii[i].nrbucati; j++) {
            matrice[linie][poz]->dimensiuni[j] = tapiserii[i].dimensiuni[j];
        }
    }
    return matrice;
}

void mutaLiniile(Tapiserie*** matrice, int nrLinii, int** nrElemLinie) {
    int m = nrElemLinie[0];
    //Numaram cate obiecte se afla pe prima linie si apoi comparam sa vedem care este cel mai mare numar de obiecte pe o linie
    for(int i=0; i< nrLinii; i++) {
        if(m < nrElemLinie[i]) {
            m = nrElemLinie[i];
        }
    }
    //Mutam liniile in functie de numarul de dimensiuni
    //Bubble sort
    for(int i=0; i<nrLinii-1; i++) {
        for(int j = 0; j< nrLinii -i - 1; j++) {
            if(nrElemLinie[j] > nrElemLinie[j+1]) {
                Tapiserie** temp = matrice[j];
                matrice[j] = matrice[j+1];
                matrice[j+1] = temp;
                int tempNr = nrElemLinie[j];
                nrElemLinie[j] = nrElemLinie[j+1];
                nrElemLinie[j+1] = tempNr;
            }
        }
   }
}

void afisareMatrice(Tapiserie*** matrice, int nrLinii, int* nrElemLinie) {
    for(int i=0; i< nrLinii; i++) {
        printf("Linia %d: ", i);
        for(int j=0; j<nrElemLinie[i]; j++) {
            afisareTapiserie(*matrice[i][j]);
        }
        printf("\n");
    }
}
...
int main() {
    int n = 10;
    Tapiserie* tapiserii = citiredinFisier("tapiserii.txt", &n);

    int* nrElemLinie = NULL;
    int nrLinii = 0;

    afisarevectorTapiserie(tapiserii, n);
    printf("Matricea:\n");
    Tapiserie*** matrice = copiereinMatrice(tapiserii, n, &nrLinii, &nrElemLinie);
    printf("Matricea a fost copiata in matrice\n");
    //Afisam matricea
    afisareMatrice(matrice, nrLinii, nrElemLinie);

    mutaLiniile(matrice, nrLinii, nrElemLinie);
    printf("Matricea a fost mutata\n");
